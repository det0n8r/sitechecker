    ...
    public partial class Main : Form
    {
        ...
        public string[] RunPSScript(string script, string[] parms, string delimiter)
        {
            if (parms != null)
            {
                if (parms.Length == 1)
                {
                    treeView1.Invoke(new LogText(logText), new object[] { "$var = " + "'" + parms[0] + "'" });
                }
                if (parms.Length > 1)
                {
                    int i = 1;
        ...
        public string[] dbConn()
        {
            ...
            string[] dbConn = RunScript(Properties.Resources.GetDBConn, null).Split(csdelim, StringSplitOptions.RemoveEmptyEntries);
            return dbConn;
        }
        string[] storages;
        string[] pvdStorages;
        ...
        string[] jasperserviceNames = { "Citrix AD Identity Service", "Citrix Broker Service", "Citrix Configuration Logging Service", "Citrix Configuration Service", "Citrix Delegated Administration Service", "Citrix Host Service", "Citrix Machine Creation Service" };
        string[] jasperbrokerProcesses = { "Citrix.ADIdentity", "BrokerService", "Citrix.ConfigurationLogging", "Citrix.ConfigurationService", "Citrix.DelegatedAdmin", "Citrix.Host", "Citrix.MachineCreation" };
        string[] serviceNames;
        string[] brokerProcesses;
        string[] vdaservices = { "WorkstationAgent", "PorticaService", "CitrixCseEngine"};
        string[] vdaProcesses = { "WorkstationAgent", "PicaSvc", "CitrixCseEngine" };
        string[] statuses = { "Completed", "Running", "Terminated" };
        string[] siteDetailsLabel = { "Site Name", "Database Server", "Database Name", "Database Version", "# of Controllers", "Service Instances" };
        ...
        private string RunScript(string scriptText, string[] param)
        {
            ...
            if (param != null)
            {
                if (param.Length == 1)
                {
                    runspace.SessionStateProxy.SetVariable("var", param[0]);
                }
                if (param.Length > 1)
                {
                    int i = 1;
        ...
        public bool buildSite(bool isnew, string[] dbconn)
        {
            ...
            brokers = RunPSScript(Properties.Resources.GetBrokers, null, "\r\n");
            expected = expected * brokers.Length;
            services = RunPSScript(Properties.Resources.GetServices, null, "\r\n");
            _siteInfo.Clear();
            siteName = RunPSScript(Properties.Resources.GetBrokersite, null, "\r\n");
            ...
            string[] siteDetailsValues = new string[] { siteName[0].ToString(), sqlServer[0].TrimStart(' '), dbconn[3].TrimStart(' '), siteVersion, brokers.Length.ToString(), totalServices.ToString() + " of " + expected };
            int x = 0;
        ...
        {            
            ...
            {                
                ...
                string[] serviceParm = new string[] { broker };
                ...
                foreach (string service in services)
                {
                    if (service.IndexOf(broker, 0, StringComparison.CurrentCultureIgnoreCase) != -1)
                    {
                        ...
                        if (service.IndexOf("ConfigurationLoggingService", 0, StringComparison.CurrentCultureIgnoreCase) != -1)
                        {
                            configLoggingServices.Add(service);
                        }
                        ...
                        if (service.IndexOf("MachineCreationService", 0, StringComparison.CurrentCultureIgnoreCase) != -1)
                        {
                            machineCreationServices.Add(service);
                        }
                        if (service.IndexOf("DelegatedAdminContract", 0, StringComparison.CurrentCultureIgnoreCase) != -1)
                        {
                            delegatedAdminServices.Add(service);
                        }
                    }
                }
                ...
                if (siteVersion == "5.0" | siteVersion == "5.1" | siteVersion == "5.6")
                {
                ...
                if (siteVersion == "6.0")
                {
                    ...
                    treeView1.Invoke(new AddNode(AddServiceNode), new object[] { serviceNames[6], machineCreationServiceBackColor });
                    foreach (string service in machineCreationServices)
                    {
                        treeView1.Invoke(new AddNode(AddServiceInstanceNode), new object[] { service, Color.White });
                    }
        ...
        public bool buildAssignments(bool isnew)
        {
            ...
            treeView1.Invoke(new AddSiteNode(AddDesktopGroupsNode), new object[] { "Desktop Groups" });
            ...
            string[] groupNames = RunPSScript(Properties.Resources.GetBrokerDesktopGroupNames, null, "\r\n");
        ...
        public void AddDesktopGroupsNode(string name)
        {
            ...
            desktopGroupsNode.ImageIndex = 10;
            //desktopGroupsNode.Expand();
        }
        ...
        void siteEnum_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            string[] results = RunPSScript(Properties.Resources.EnumSite, null, null);
        ...
        void nodeClick_DoWork(object sender, DoWorkEventArgs arg)
        {
            ...
            if (lastNodeClicked.Text != e.Node.Text && lastNodeClicked.Text != null && e.Node != null && e.Node == nodeclicked)
            {
                ...
                if (e.Node.Level == 0)
                {
                    string[] siteDetails = RunPSScript(Properties.Resources.GetBrokersiteFull, null, "\r\n");
            ...
            if (e.Node.Level == 3)
            {
                if (e.Node.Parent.Text.Contains("Desktop Groups")) //Check if the clicked node is a hosting unit
                {
                    string[] parms = new string[] { e.Node.Text.Trim() };
                ...
                if (e.Node.Parent.Text.Contains("Controllers"))// | e.Node.Parent.Text.Contains("Service")) //Check if the clicked node is a DDC
                {
                    if (!badBrokers.Contains(e.Node.Text))
                    {
                        ...
                        string[] parms = new string[] { e.Node.Text.Trim(), e.Node.Parent.Text };                        
                ...
                if (e.Node.Parent.Text.Contains("Catalogs")) //Check if the clicked node is a hosting unit
                {
                    string[] parms = new string[] { e.Node.Text.Trim() };
                ...
                if (e.Node.Parent.Text.Contains("AD Identity")) //Check if the clicked node is a hosting unit
                {
                    string[] parms = new string[] { e.Node.Text.Trim() };
                ...
                if (e.Node.Parent.Text.Contains("Schemes")) //Check if the clicked node is a hosting unit
                {
                    string[] parms = new string[] { e.Node.Text.Trim() };
            ...
            {                
                ...
                {
                    string[] parms = new string[] { e.Node.Text };
                ...
                if (e.Node.Parent.Parent.Text.Contains("Sessions"))
                {
                    string[] split = e.Node.Text.Split(':');
                    string[] parms = new string[] { split[1], e.Node.Parent.Parent.Text };
                ...
                {
                    string[] parms = new string[] { e.Node.Text, e.Node.Parent.Parent.Text };
                ...
                {
                    string[] parms = new string[] { e.Node.Parent.Parent.Text.Trim() };
                    string[] hostingUnits = RunPSScript(Properties.Resources.GetBrokerHypervisorConnection, parms, "\r\n");
                    treeView1.Invoke(new SetNodeInfo(setNodeInfo), new object[] { hostingUnits });
                    //treeView1.Invoke(new SetNode(setNode), new object[] { Properties.Resources.HostServiceInfo, "rt" });
                }
                ...
                {
                    string[] parms = new string[] { e.Node.Parent.Text.Trim(), e.Node.Text };
                ...
                if (e.Node.Parent.Parent.Text.Contains("Power")) //Add the site services and roles descriptions
                {
                    ...
                    string[] parms = new string[] { splitDetails[1] , "Power"};
                    string[] serviceInstanceDetails = RunPSScript(Properties.Resources.GetObject, parms, "\r\n");                    
                    treeView1.Invoke(new SetNodeInfo(setNodeInfo), new object[] { serviceInstanceDetails });
                } 
            }
            ...
            {
                if (e.Node.Parent.Parent.Parent.Text.Contains("Desktop Groups"))
                {
                    string[] parms = new string[] { e.Node.Text.Trim(), e.Node.Parent.Parent.Parent.Parent.Text };
                ...
                if (e.Node.Text.Contains("Citrix"))
                {
                    string[] parms = new string[] { e.Node.Text, "ServiceInstance" };
                    string[] serviceInstanceDetails = RunPSScript(Properties.Resources.GetObject, parms, "\r\n");
                    treeView1.Invoke(new SetNodeInfo(setNodeInfo), new object[] { serviceInstanceDetails });
                }                
            }
        ...
        void populateTree_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            switch (arg[0])
            {
                case "Configuration":
                    ...
                    hostingUnits = RunPSScript(Properties.Resources.GetHostsByName, null, "\r\n");
                    storages = RunPSScript(Properties.Resources.GetStorages, null, "\r\n");
                    ...
                    {                        
                        if (confignodes.Text.Contains("Hosts"))
                        {
                            ...
                            {                            
                                ...
                                if (siteVersion.Contains("5.6"))
                                {
                                    pvdStorages = RunPSScript(Properties.Resources.GetPvdStorages, null, "\r\n");
                                ...
                                string[] networks = RunPSScript(Properties.Resources.GetNetworks, null, "\r\n");
                    ...
                    break;
                case "Sessions":
                    string[] activeSessions = RunPSScript(Properties.Resources.GetActiveSessions, null, "\r\n");
                ...
                case "Machines":
                    ...
                    string[] details = RunPSScript(Properties.Resources.GetMachineInfo, null, "\r\n");
                ...
                case "Active Power Actions":
                    ...
                    Thread.Sleep(100);
                    string[] pendingPowerActions = RunPSScript(Properties.Resources.GetPendingPowerActions, null, "\r\n");
                ...
                case "Completed Power Actions":
                    ...
                    string[] completePowers = RunPSScript(Properties.Resources.GetCompletePowerActions, null, "\r\n");
        ...
        void getDesktopStats_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            string[] desktops = RunScript(Properties.Resources.GetBrokerDesktop, null).Replace("\r\n", " ").Split(new string[] { "#" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        private void buildTree_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            try
                {
                toolStripProgressBar1.Visible = false;
                siteInfoListView.View = View.Details;
                siteInfoListView.Items.Clear();
                siteInfoListView.HeaderStyle = ColumnHeaderStyle.None;
                getDesktopStats.RunWorkerAsync();
        ...
        private void buildTree_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            List<string> localHost = new List<string> { fqdn };
            Assembly assembly = Assembly.GetExecutingAssembly();
            FileVersionInfo fvi = FileVersionInfo.GetVersionInfo(installPath("localhost") + "\\Broker\\Service\\BrokerService.exe");
            siteVersion = Left(fvi.FileVersion, 3);            
            if (siteVersion == "5.0" | siteVersion == "5.6" | siteVersion == "5.1")
            {
                expected = 12;
        ...
        private void runScript_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            string[] parms = new string[] { nodeclicked.Text };
        ...
        private void serviceReset_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (arg == "clear preferred")
            {
                ...
                string[] parms = new string[] { nodeclicked.Text.Trim() };
        ...
        private void removeStorageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                string[] parms = { "removeStorage", nodeclicked.Parent.Parent.Text.Trim(), nodeclicked.Text.Trim() };
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            string[] areModulesLoaded = RunPSScript(Properties.Resources.CheckNeededSnapins, null, "\r\n");
            if (areModulesLoaded[0] == "Yes")
            {
                ...
                while (ddcLocation.Contains("Remote"))
                {
                    runlocal = false;                    
                    parm[0] = targetDDC;
                    ddcLocation = RunScript(Properties.Resources.TestSiteConn, parm);
                    if (ddcLocation.Contains("Remote"))
                    {
                        targetDDC = Interaction.InputBox("XenDesktop SDK Installed. Specify target DDC:", "Remote connect", "DDC FQDN");
                        if (targetDDC.Contains("DDC FQDN"))
                        {
                            targetDDC = Interaction.InputBox("Please specify target DDC FQDN or IP address:", "Remote connect", "");
                        }
                        if (targetDDC.Contains(""))
                        {
                            //canceled
                            this.Close();   
                        }
                    }
                    else if (ddcLocation.Contains("Local"))
                    {
                        //Good
                    }
                }
                if (ddcLocation.Contains("Local"))
                {
        ...
        {
            string[] dbconn = dbConn();
            string[] scriptParms = new string[] { nodeclicked.Text };
            string[] brokerSID = RunPSScript(Properties.Resources.GetBrokerSid, scriptParms, "\r\n");
            string[] parms = { "evictScript", nodeclicked.Text, brokerSID[0], dbconn[3].Trim(), string.Format("XDSC-{0:yyyy-MM-dd_hh-mm-ss-tt}.txt", DateTime.Now) };
            if (!workProgress.IsBusy)
            {
                workProgress.RunWorkerAsync(parms);
            }
        ...
        private void treeView1_AfterSelect(object sender, TreeViewEventArgs e)
        {
            if (e.Node == nodeclicked)
            {
                ...
                if (e.Node.Level > 1)
                {
                    if (e.Node.Parent.Text == "Desktop Groups")
                    {
        ...
        {            
            ...
            if (e.Node.Text == "Sessions")
            {
                ...
                foreach (TreeNode node in inactiveSessionsNode.Nodes)
                {
                    node.Remove();
                }
                string[] parms = { e.Node.Text };
        ...
        {
            if (nodeclicked.Parent.Parent.Text == "Sessions")
            {
                string[] split = nodeclicked.Text.Split(':');
                string[] param = new string[] { split[1] };
