...
namespace XDSiteChecker
{
	public partial class CheckWorkers : Form
	{
        ...
        public string ReadKey (string[] args)
        {
            RegistryKey environmentKey;
            string remoteName = args[0];
            string keytocheck = args[1];
            string valuetocheck = args[2];
            string valueData = "";
            try
            {
                environmentKey = RegistryKey.OpenRemoteBaseKey(RegistryHive.LocalMachine, remoteName).OpenSubKey(args[1]);
                foreach (string valueName in environmentKey.GetValueNames())
                {
                    if (valueName.Contains(args[2]))
                    {
                        valueData = environmentKey.GetValue(valueName).ToString();
                    }
                    else
                    {
                        valueData = "Not Found";
            ...
            {
                valueData = e.InnerException.ToString();             
            }
            return valueData;            
        }            
								...
								if (column.OwningColumn.Name == "PowerState")
								{
									if (column.Value.ToString() != "Off")
									{
										if (isOnline(machine) == true)
										{
                                            ...
                                            if (os.Contains("XP"))
                                            {
                                                string[] parms = new string[] { machine, "Software\\Policies\\Citrix\\VirtualDesktopAgent", "ListOfDDCs" };
                                                listOfDDCs = Program.RunScript2(Properties.Resources.GetRegValue, parms);
                                                if (listOfDDCs.Contains("Not Found"))
                                                {
                                                    parms = new string[] { machine, "Software\\Citrix\\VirtualDesktopAgent", "ListOfDDCs" };    
                                                    listOfDDCs = Program.RunScript2(Properties.Resources.GetRegValue, parms);
                                                    if (listOfDDCs.Contains("Not Found"))
                                                    {
                                                        parms = new string[] { machine, "Software\\Wow6432Node\\Policies\\Citrix\\VirtualDesktopAgent", "ListOfDDCs" }; 
                                                        listOfDDCs = Program.RunScript2(Properties.Resources.GetRegValue, parms);
                                                        if (listOfDDCs.Contains("Not Found"))
                                                        {                                                        
                                                            parms = new string[] { machine, "Software\\Wow6432Node\\Citrix\\VirtualDesktopAgent", "ListOfDDCs" };  
                                                            listOfDDCs = Program.RunScript2(Properties.Resources.GetRegValue, parms);
                                                        }
                                                    }
                                                }
                                                parms = new string[] { machine, "Software\\Microsoft\\Windows NT\\CurrentVersion\\Perflib\\009", "Counter" };
                                                perfmonCorrupt = Program.RunScript2(Properties.Resources.GetRegValue, parms); 
                                                if (perfmonCorrupt == "\r\n\r\n")
                                                {
                                                    perfmonCorrupt = "Corrupt";
                                                }
                                                else { perfmonCorrupt = "OK"; } 
                                            }
                                            // If not WinXP
                                            else
                                            {
                                                ...
                                                {
                                                    listOfDDCs = "Access Denied";
                                                } 
                                                try
                                                {
                                                    string[] oValue = null;
                                                    ManagementBaseObject inParams = mc.GetMethodParameters("GetMultiStringValue");
                                                    inParams["sSubKeyName"] = "Software\\Microsoft\\Windows NT\\CurrentVersion\\Perflib\\009";
                                                    inParams["sValueName"] = "Counter";
                                                    ManagementBaseObject outParams = mc.InvokeMethod("GetMultiStringValue", inParams, null);
                                                    if (Convert.ToUInt32(outParams["ReturnValue"]) == 0)
                                                    {
                                                        oValue = outParams["sValue"] as string[];
                                                        if (oValue[0] == "")
                                                        {
                                                            perfmonCorrupt = "Corrupt";
                                                        }
                                                ...
                                                catch { perfmonCorrupt = "Access Denied"; }
                                            }											
											
											//string[] parms2 = new string[] { machine };
											//Invoke(new RefreshForm(refreshForm), new object[] { });
											string workerResults = icaServiceStatus + "," + last.ToString() + "," + uptime + "," + desktopServiceStatus + "," + listOfDDCs + "," + perfmonCorrupt;
										...
										{
											string workerResults = "Offline,Offline,Offline,Offline,Offline,Offline";
											string[] workerSplit = workerResults.Split(',');
											Invoke(new UpdateWorker(updateWorker), new object[] { workerSplit, machine });
										}
									}
									...
									{
										string workerResults = "Off,Off,Off,Off,Offline,Offline";
										string[] workerSplit = workerResults.Split(',');
										Invoke(new UpdateWorker(updateWorker), new object[] { workerSplit, machine });
									}
								}
		...
		private void colorCell(DataGridViewCell e)
		{
			try
			{
                ...
                if (e.ColumnIndex == this.dataGridView1.Columns["PerfmonLibrary"].Index)
                {
                    string corrupt = e.Value.ToString().Replace("\r\n", "");
                    if (corrupt != null)
                    {
                        switch (corrupt)
                        {
		...
		public void updateWorker (string[] workerdata, string machine)
		{
			...
			foreach (worker worker in workers)
			{
				...
				{
					worker.ICAService = workerdata[0].Trim();
					worker.LastBootTime = workerdata[1];
					worker.Uptime = workerdata[2];
					worker.DesktopService = workerdata[3];
					worker.ListOfDDCs = workerdata[4];
                    worker.PerfmonLibrary = workerdata[5];
		...
		public void addWorker (string[] workerSplit)
		{
            workers.Add(new worker
            {
                ...
                LastUser = workerSplit[7],
                LastConnectionTime = workerSplit[8],
                LastPowerTime = workerSplit[9],
                LastDeregReason = workerSplit[10],
                LastDeregTime = workerSplit[11],
                LastBootTime = workerSplit[12],
                Uptime = workerSplit[13],
                DesktopService = workerSplit[14],
                ListOfDDCs = workerSplit[15],
                PerfmonLibrary = workerSplit[16]
            }); //build a worker                
		}        
		...
		{            
            ...
            public string PerfmonLibrary { get; set; }
		}
		...
		public void dataGridView1_ColumnHeaderMouseClick(object sender, System.Windows.Forms.DataGridViewCellMouseEventArgs e)
		{
